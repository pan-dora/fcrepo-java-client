 ...
 * <p>
 * Users of the {@code FcrepoClient} are responsible for managing connection resources.
 * Specifically, the underlying
 * HTTP connections of this client must be freed. Suggested usage is to create the {@code
 * FcrepoResponse} within a
 * {@code try-with-resources} block, insuring that any resources held by the response are freed
 * automatically.
 * </p>
 * <pre>
 * FcrepoClient client = ...;
...
public class FcrepoClient {
     ...
     * @param throwExceptionOnFailure whether to throw an exception on any non-2xx or 3xx HTTP
     * responses
     */
    protected FcrepoClient(final String username, final String password, final String host,
        ...
        final FcrepoHttpClientBuilder client = new FcrepoHttpClientBuilder(username, password,
                host);
    ...
    /**
     * Make a PUT request to create a resource with a specified path, or replace the triples
     * associated with a
     ...
     * @throws FcrepoOperationFailedException when the underlying HTTP request results in an error
     */
    public CopyBuilder copy(final URI source, final URI destination) throws
     ...
     * @throws FcrepoOperationFailedException when the underlying HTTP request results in an error
     */
    public MoveBuilder move(final URI source, final URI destination) throws
            FcrepoOperationFailedException {
     ...
     */
    private FcrepoResponse fcrepoGenericResponse(final URI url, final CloseableHttpResponse
            response,
                                                 final Boolean throwExceptionOnFailure) throws
            FcrepoOperationFailedException {
    ...
    /**
     * Frees resources associated with the HTTP response. Specifically, closing the {@code
     * response} frees the
     * connection of the {@link org.apache.http.conn.HttpClientConnectionManager} underlying this
     * {@link #httpclient}.
     *
