 ...
 * <p>
 * This class implements {@link Closeable}. Suggested usage is to create the {@code
 * FcrepoResponse} within a
 * try-with-resources block, insuring that any resources held by the response are freed
 * automatically.
 * </p>
 * <pre>
 * FcrepoClient client = ...;
...
public class FcrepoResponse implements Closeable {
     ...
     */
    public FcrepoResponse(final URI url, final int statusCode, final Map<String, List<String>>
            headers,
     ...
     * {@inheritDoc}
     * <p>
     * Implementation note: Invoking this method will close the underlying {@code InputStream}
    ...
    /**
     * Whether or not the resources have been freed from this response. There should be no
     * expectation that a closed
    ...
    public List<URI> getLinkHeaders(final String relationship) {
        return getHeaderValues(LINK).stream().map(FcrepoLink::new).filter(link -> link.getRel()
                .equals(relationship))
