...
public class GetBuilder extends
     ...
     *
     * @param mediaType media type to set as the accept header. It should be a value from one of
     * the allowed RDF
    ...
    /**
     * Set the prefer header for this request to minimal, to indicate that only triples directly
    ...
    /**
     * Set the prefer header for this request to representation, to indicate that links to other
     * resources and their
     ...
     * Set the prefer header for this request to representation, to indicate that links to other
     * resources and their
     * properties should also be included. The set of properties returned can be further
     * specified by providing lists
    ...
    private String buildPrefer(final String prefer, final List<URI> includeUris, final List<URI>
            omitUris) {
        final StringJoiner preferJoin = new StringJoiner("; ");
        preferJoin.add("return=" + prefer);

        if (includeUris != null) {
            final String include = includeUris.stream().map(URI::toString).collect(Collectors
                    .joining(" "));
            if (include.length() > 0) {
