...
/**
 * Builds a PUT request for interacting with the Fedora HTTP API in order to create a resource
...
public class PutBuilder extends BodyRequestBuilder {
    ...
    public PutBuilder filename(final String filename) throws FcrepoOperationFailedException {
        try {
            final String f = (filename != null) ? "; filename=\"" + URLEncoder.encode(filename,
                    "utf-8") + "\"" : "";
    ...
    /**
     * Set the prefer header for this request to lenient handling, to indicate that
     * server-managed triples will not
