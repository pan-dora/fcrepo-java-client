...
public class FcrepoClientErrorTest {
    ...
    public void testHeaders() throws IOException, FcrepoOperationFailedException {
        ...
        final Header linkHeader = new BasicHeader("Link", "<" + baseUrl + "/bar>; " +
                "rel=\"describedby\"");
        final Header linkFooHeader = new BasicHeader("Link", "<" + baseUrl + "/bar>; rel=\"foo\"");
        final Header[] headers = new Header[]{locationHeader, contentTypeHeader, linkHeader,
                linkFooHeader};
    ...
    private void doSetupMockRequest(final String contentType, final ByteArrayEntity entity, final
    int status)
            throws IOException {
