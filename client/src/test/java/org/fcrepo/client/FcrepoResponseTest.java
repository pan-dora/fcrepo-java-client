...
public class FcrepoResponseTest {
    ...
    /**
     * Demonstrates that response objects are <em>not</em> {@code close()}ed by default, that the
     * state of
     * {@link FcrepoResponse#closed} is set appropriately when {@link FcrepoResponse#close()} is
     * invoked under normal
     * (i.e. no exception thrown during {@code close()}) conditions, and that
     * {@link InputStream#close()} is not
    ...
    public void testClosableReleasesResources() throws IOException {
        ...
        assertFalse("FcrepoResponse objects should not be closed until close() is invoked.",
                underTest.isClosed());
    ...
    /**
     * Demonstrates that if an {@code IOException} is thrown by {@link FcrepoResponse#close()},
     * <em>and</em> an
     * exception is thrown inside of a client's {@code try} block, the {@code IOException} from
     * the {@code close()}
